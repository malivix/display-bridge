...
func runMenuSelfTests() {
    ...
    precondition(sizeComparison(comparisonCurrent,comparisonLarger,details:true).contains("3840 × 2160 → 3072 × 1728"))
    precondition(!sizeComparison(comparisonCurrent,comparisonLarger).contains("Framebuffer"))
    precondition(sizeComparison(["physical_size_percent":100],["physical_size_percent":154.3]).contains("PG about 54% larger than BenQ"))
    precondition(sizeComparison(["physical_size_percent":100],["physical_size_percent":100]).contains("PG and BenQ about the same size"))
    print("PASS compact size comparison, direction, unknown values and optional framebuffer details")
