...
static DDCValue readingOperation(DDCTransport *transport, DDCPacket *packet) {
    DDCValue failure = {-1, -1};
    for (int attempt = 0; attempt < 3; attempt++) {
        // One Get-VCP request per complete transaction. Do not duplicate a
        // request while the monitor is still preparing its first response.
        if (attempt > 0) usleep(50000);
        prepareDDCRead(packet);
        IOReturn err = IOAVServiceWriteI2C(transport->service, transport->chipAddress,
                                         packet->inputAddr, packet->data, 4);
        if (err) continue;
        DDCPacket reply = {};
        reply.inputAddr = packet->inputAddr;
        err = performDDCReadAtChipAddress(transport->service, transport->chipAddress, &reply);
        if (err || !validVCPReply(reply.data, packet->data[2])) continue;
        return convertI2CtoDDC((char *)reply.data);
    }
    fprintf(stderr, "No valid matching DDC reply after three attempts\n");
    return failure;
}
...
static UInt8 attrCodeFromCommand(char *command) {
    ...
    else if (STR_EQ(command, "orientation")) { return 0xaa; }
    return 0x00;
}
...
int main(int argc, char** argv) {
    ...
    if (attrCode == 0xaa && !STR_EQ(argv[0], "get")) { writeToStdOut(@"Orientation is read-only\n"); return EXIT_FAILURE; }
